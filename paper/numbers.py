...
def emit_tex(N: dict) -> str:
    ...
    d("channelList", ", ".join(f"\\texttt{{{x}}}" for x in c["channels"]))
    d("variantList", ", ".join(f"\\texttt{{{x}}}" for x in c["variants"]))

    o = N["oracle"]
    d("nEncodings", o["n_encodings"]); d("nOracleTests", o["n_tests"])
    d("encodingList", ", ".join(f"\\texttt{{{x.replace('_', chr(92) + '_')}}}" for x in o["encodings"]))
    d("egressToolList", ", ".join(f"\\texttt{{{x.replace('_', chr(92) + '_')}}}" for x in o["egress_tools"]))
    d("nEgressTools", len(o["egress_tools"]))

    k = N["canary"]
    d("canaryID", f"\\texttt{{{k['id']}}}")
    d("nCustomers", k["n_customers"]); d("nRecordFields", k["n_record_fields"])
    d("nPhiFieldsObserved", k["n_phi_fields_observed"])
    d("phiFieldList", ", ".join(f"\\texttt{{{x.replace('_', chr(92) + '_')}}}" for x in k["phi_fields_observed"]))
    ...
    d("unpromptedFieldList", ", ".join(f"\\texttt{{{x.replace('_', chr(92) + '_')}}}" for x in u["phi_fields"]))

    d("apiCalls", N["budget"]["api_calls"]); d("cacheEntries", N["budget"]["cache_entries"])
    d("configFingerprint", f"\\texttt{{{N['provenance']['fingerprint']}}}")
    d("attackerHost", f"\\texttt{{{N['provenance']['attacker_host']}}}")
