...
\emph{Panel size and tier confound.} \nModelsWord{} models, one per category, with a
large tier and scale mismatch; see \cref{sec:refuse}.

\emph{Single trial per cell.} One run per cell at temperature~\temperature{}. Hosted
temperature-\temperature{} inference is not exactly deterministic---we observed one benign
...
\emph{Fixed encoding list.} \nEncodingsWord{} encodings; a novel recoverable encoding
