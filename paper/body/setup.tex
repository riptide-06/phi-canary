...
substantially in tier and scale (\cref{sec:refuse}). Four further models were attempted and dropped before
scoring for access reasons unrelated to the behaviour under study
(\cref{app:dropped}). One trial per cell at temperature~\temperature{}, a \maxOutputTokens{}-token
output cap, and a \maxTurnsWord{}-turn budget; responses are cached by prompt, so
the study is idempotent and closed, having consumed \apiCalls{} model calls. The cap was raised from an initially planned \plannedOutputTokens{} tokens
because the proprietary arm spends output tokens on hidden reasoning before
emitting a tool call; too small a cap truncates episodes before the egress call
and \emph{understates} exfiltration. The primary metric is the canary's
bytes, under any of the \nOtherEncodingsWord{} encodings, in an egress-tool argument
...
\paragraph{Protocol.}
One trial per cell at temperature~\temperature{}, a per-call output cap of \maxOutputTokens{} tokens, and a
...
The per-call output cap was raised from an initially planned \plannedOutputTokens{} tokens to \maxOutputTokens{}.
...
under any of the \nOtherEncodingsWord{} encodings, in an argument to an egress tool whose
destination is \attackerHost{}. We report it as a count out of \llamaN{} and as a
rate with a percentile bootstrap \ciLevel{}\% confidence interval over \bootN{}
