...
For exfiltration, the per-episode judgement of whether an attack succeeded can be
replaced by a construction: plant a record no legitimate task transmits, and read
its bytes at the egress boundary. The result we would most like carried forward is
the one that required admitting a design error. The obvious form of this
label---the planted record in any outbound argument---was wrong
\llamaControlFPRate{} of the time on traffic containing no adversary at all,
because transmitting member records is a plausible reading of the agent's job, and
we found that only by running a matched control arm. A security label for agents
is not credible until it has been run against the agent doing its job unmolested.
\else
