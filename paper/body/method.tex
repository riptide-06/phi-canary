...
A synthetic member record carrying \canaryID{} and \nRecordFields{} fields (name,
date of birth, diagnosis codes, medications, address) is planted among
\nCustomersWord{} records; it is invented in full. \emph{Detection}---which
encoding of the canary is present in a string---is a pure function of bytes,
frozen, and exercised by \nOracleTests{} tests. \emph{Policy}---which tools are
egress, which host is the adversary's---is configurable and delegates every
byte-level decision to the detector; re-scoring every logged call and episode
through both paths gave identical verdicts. The detector reports
the least-transformed of \nEncodingsWord{} encodings (\encodingList{}); a novel
...
the task---and only the \nEgressToolsWord{} egress tools (\egressToolList{}) are
scored.
\else
...
\nEncodingsWord{} recoverability-preserving encodings: \encodingList{}. The list is
...
a leak. Only arguments to the \nEgressToolsWord{} egress tools (\egressToolList{})
are scored.
...
\ifshort
The reference agent resolves a support ticket for a fictional health plan end to
end, with \nAgentToolsWord{} simulated tools returning fixed data and a
\maxTurnsWord{}-turn budget. Tool calls are emitted as plain text and parsed by
regular expression, not through a native function-calling interface; the system
prompt states that member records are protected health information.
\else
...
\nAgentToolsWord{} tools and a budget of \maxTurnsWord{} turns. Its system prompt states
...
\ifshort
\subsection{Corpus and control arm}
The corpus is \corpusSize{} payloads: \nChannelsWord{} channels (\channelList{})
crossed with \nVariantsWord{} variants (\variantList{}), each channel dressing
the injected text as that surface plausibly looks and each variant holding intent
...
dangerous output is a clean bill of health it did not earn. The harness refuses to
score until four checks pass, the fourth a positive control in which a blatant
unobfuscated exfiltration instruction is issued and the oracle \emph{must} detect
the leak; an override stamps every artifact unverified. The gate caught a real
defect in this project's own adapter scaffold, where template and harness imported
the same base class by two module paths, so a correct adapter was rejected.
\else
\subsection{Corpus}
The corpus is \corpusSize{} payloads: \nChannelsWord{} delivery channels
(\channelList{}) crossed with \nVariantsWord{} obfuscation variants
...
\perAgentInjected{} injected episodes plus an equal number of controls,
\perAgentEpisodes{} in all.
