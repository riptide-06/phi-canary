...
the adapter interface, not demonstrated. Tools are simulated and tool calls parsed
from plain text rather than issued natively. The panel is \nModelsWord{} models,
one per category, with license and tier confounded (\cref{sec:refuse}); one trial
per cell at temperature~\temperature{}, with intervals that resample payloads and
so do not capture run-to-run variability; \corpusSize{} payloads, English only;
and a fixed encoding list, so rates are lower bounds. The full list, including
test coverage and the canary's unmeasured observer effect, is
...
\nModelsWord{} models, one per deployment category, with a large tier and scale
...
Each cell is run once at temperature~\temperature{}. Hosted inference at temperature~\temperature{} is not
...
The detector covers \nEncodingsWord{} encodings. A model that invented a novel
